...
void main() {
  group('$JwtStrategy', () {
    test('should have a name of jwt', () {
      final strategy = JwtStrategy(
          JwtStrategyOptions(SecretKey(''),
              jwtFromRequest: ExtractJwt.fromHeaders('auth')),
    ...
    test('should authenticate a token', () async {
      final strategy = JwtStrategy(
          JwtStrategyOptions(SecretKey('hello'),
    ...
    test('should throw an error if the token is invalid', () async {
      final strategy = JwtStrategy(
          JwtStrategyOptions(SecretKey('hallo'),
              jwtFromRequest: ExtractJwt.fromAuthHeaderAsBearerToken()),
          (options, jwt, done) async {
        done(jwt);
      });
    ...
    test(
        'should parse header and return the schema and the value of the header',
        () {
